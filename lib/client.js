...
client.prototype.request = function(method, path, params, callback){
    ...
    var protocol = '';
    if ( http_options.hostname.indexOf('http://localhost:') > -1 ) {
      protocol = http;
      http_options.port = this.api_host.split(':')[2];
      const host = http_options.hostname;
      const lastIndex = host.lastIndexOf(':');
      http_options.hostname = host.substring(0, lastIndex);
    } else {
      protocol = https;
    }

    var req = protocol.request(http_options, function(res){
